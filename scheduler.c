...
void SchedDestroy(sched_t *sched)
{
    ...
    SchedFlush(sched);
    PQDestroy(sched->p_queue); sched->p_queue = NULL;
    free(sched->curr_task);
...
int SchedRun(sched_t* sched)
{
    ...
    /*set start_time to current time, with protection for time() func*/
    ...
    {
        /*set 'now' to current time, with protection for time() func*/
        for(now = time(NULL); -1 == now; now = time(NULL));

        sched->curr_task = PQPeek(sched->p_queue);
        PQDequeue(sched->p_queue);
        /*sleep the needed time, with protection for sleep() func*/
